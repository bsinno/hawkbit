...
public class TagWindowLayoutComponentBuilder {
    ...
    public TextField createNameField(final String id, final Binder<ProxyTag> binder) {
        final TextField tagName = new TextFieldBuilder(NamedEntity.NAME_MAX_SIZE).id(id)
        ...
        binder.forField(tagName).asRequired("You must provide tag name").bind(ProxyTag::getName, ProxyTag::setName);

        return tagName;
    }

    public TextArea createDescription(final String id, final Binder<ProxyTag> binder) {
        final TextArea tagDescription = new TextAreaBuilder(NamedEntity.DESCRIPTION_MAX_SIZE).id(id)
        ...
        binder.forField(tagDescription).bind(ProxyTag::getDescription, ProxyTag::setDescription);
