 ...
 */
public class SwModuleGrid extends AbstractGrid<ProxyAssignedSoftwareModule, SwDistributionsFilterParams> {
    ...
    private final ConfigurableFilterDataProvider<ProxyAssignedSoftwareModule, Void, SwDistributionsFilterParams> swModuleDataProvider;
    private final AssignedSoftwareModuleToProxyMapper assignedSoftwareModuleToProxyMapper;
    private final DeleteSupport<ProxyAssignedSoftwareModule> swModuleDeleteSupport;
    private final DragAndDropSupport<ProxyAssignedSoftwareModule> dragAndDropSupport;
    ...
    public SwModuleGrid(final UIEventBus eventBus, final VaadinMessageSource i18n,
        ...
        setSelectionSupport(new SelectionSupport<ProxyAssignedSoftwareModule>(this));
    ...
    @Override
    public ConfigurableFilterDataProvider<ProxyAssignedSoftwareModule, Void, SwDistributionsFilterParams> getFilterDataProvider() {
    ...
    private void refreshFilter() {
        final SwDistributionsFilterParams filterParams = new SwDistributionsFilterParams(getSearchTextFromUiState(),
    ...
    public void updateSwModule(final ProxySoftwareModule updatedSwModule) {
        if (updatedSwModule != null) {
            ...
            // in event
            getDataProvider().refreshItem((ProxyAssignedSoftwareModule) updatedSwModule);
        }
    }
    ...
    public void addColumns() {
        // TODO: check width
        addColumn(ProxyAssignedSoftwareModule::getName).setId(SM_NAME_ID).setCaption(i18n.getMessage("header.name"))
                .setMinimumWidth(100d).setMaximumWidth(150d).setHidable(false).setHidden(false);

        addColumn(ProxyAssignedSoftwareModule::getVersion).setId(SM_VERSION_ID)
                .setCaption(i18n.getMessage("header.version")).setMinimumWidth(50d).setMaximumWidth(100d)
                .setHidable(false).setHidden(false);

        addActionColumns();

        addColumn(ProxyAssignedSoftwareModule::getCreatedBy).setId(SM_CREATED_BY_ID)
                .setCaption(i18n.getMessage("header.createdBy")).setHidable(true).setHidden(true);

        addColumn(ProxyAssignedSoftwareModule::getCreatedDate).setId(SM_CREATED_DATE_ID)
                .setCaption(i18n.getMessage("header.createdDate")).setHidable(true).setHidden(true);

        addColumn(ProxyAssignedSoftwareModule::getLastModifiedBy).setId(SM_MODIFIED_BY_ID)
                .setCaption(i18n.getMessage("header.modifiedBy")).setHidable(true).setHidden(true);

        addColumn(ProxyAssignedSoftwareModule::getModifiedDate).setId(SM_MODIFIED_DATE_ID)
                .setCaption(i18n.getMessage("header.modifiedDate")).setHidable(true).setHidden(true);

        addColumn(ProxyAssignedSoftwareModule::getDescription).setId(SM_DESC_ID)
                .setCaption(i18n.getMessage("header.description")).setHidable(true).setHidden(true);

        addColumn(ProxyAssignedSoftwareModule::getVendor).setId(SM_VENDOR_ID)
                .setCaption(i18n.getMessage("header.vendor")).setHidable(true).setHidden(true);
    }
